...
- Smart Rebalance (in Vault) computes a composite score per pool and applies safety gates (cooldown, score delta, expected gain vs cost). If gates pass, Vault executes the rebalance.
...
### 1.5 Key features
- Smart Rebalance with a composite score (details: docs/smart_rebalance.md)
...
#### 2.1.1 Vault
  ...
  - Hosts Smart Rebalance decision and applies safety gates before moving funds.
